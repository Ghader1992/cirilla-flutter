...
class _ItemState extends State<Item> with Utility, CartMixin, SnackMixin, ShapeMixin {
  ...
  Widget build(BuildContext context) {
    ...
    String? price = get(widget.cartItem.prices, ['price'], '0');
    ...
    return ProductCartItem(
        ...
        children: [
          ...
          if (widget.cartItem.variation != null)
            ...List.generate(
              ...
              (index) {
                Map variation = widget.cartItem.variation![index];
      ...
      price: Text(
        convertCurrency(context, currency: currencyCode, price: price, unit: unit)!,
        style: textTheme.titleSmall,
      ),
  ...
  Widget _buildImage() {
    List<Map<String, dynamic>>? images = widget.cartItem.images;
    return CirillaCacheImage(
      images?.isEmpty == true ? Assets.noImageUrl : images?[0]['thumbnail'] ?? Assets.noImageUrl,
      width: 86,
      height: 102,
      fit: BoxFit.cover,
    );
