...
import 'widgets/product_custom_widget.dart';
...
class _ProductScreenState extends State<ProductScreen>
  ...
  Map<String, String?> _handleWooBookingData() {
    ...
    List<String> endDate = get(_appointments, ['endDate'], '').split('-');
    ...
    try {
      ...
      String resource = get(_appointments, ['wc_bookings_field_resource'], '').toString();
      switch (type) {
        ...
        case WooBookingType.customDay:
          data['wc_bookings_field_duration'] = get(_appointments, ['wc_bookings_field_duration'], '').toString();
          if(resource != ''){
            data['wc_bookings_field_resource'] = resource;
            data['min_date'] = get(_appointments, ['min_date'], '');
            data['max_date'] = get(_appointments, ['max_date'], '');
          }
          break;
      ...
      if (type != WooBookingType.customMonth) {
        ...
        if(endDate.length == 3 && resource != ''){
          data['wc_bookings_field_start_date_to_year'] = endDate[0];
          data['wc_bookings_field_start_date_to_month'] = endDate[1];
          data['wc_bookings_field_start_date_to_day'] = endDate[2];
        }
      }
    } catch (_) {}
  ...
  Widget build(BuildContext context) {
    if (widget.store?.loading == true && widget.store?.data == null) {
      return Scaffold(
        body: Center(child: buildLoading(context, isLoading: widget.store!.loading)),
      );
    }
  ...
  List<Widget> buildColumn(
    ...
    return columns.map((e) {
      ...
      bool productShowOutOfStock = get(configData, ['productShowOutOfStock'], false);

      double productWidthImage = ConvertData.stringToDouble(get(configData, ['productWidthImage'], 38));
      double productHeightImage = ConvertData.stringToDouble(get(configData, ['productHeightImage'], 38));

      Size imageAttriSize = Size(productWidthImage, productHeightImage);
  ...
  Widget buildBlock(
    String? type, {
    bool productShowOutOfStock = false,
    Size imageAttriSize = const Size(38, 38),
    ...
    switch (type) {
      ...
      case ProductBlocks.customWidget:
        return ProductCustomWidget(
          product: _product,
          type: configData["keyCustom"] is String? ? configData["keyCustom"] : null,
          dataJson: configData["dataJson"] is String? configData["dataJson"] : '{}',
        );
      default:
        return Text(type!);
    }
  ...
  Widget buildSlideshow(WidgetConfig configs) {
    ...
    return ProductSlideshow(
      images: _variationStore != null &&
          ...
          : _variationStore?.selectedImage.isNotEmpty == true ? [_variationStore!.selectedImage] : [],
