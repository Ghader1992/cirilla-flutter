    ...
    if (url?.endsWith('.svg') == true) {
      return SvgPicture.network(
        ...
        colorFilter: colorFilter != null
            ? ColorFilter.mode(colorFilter!, BlendMode.color)
            : const ColorFilter.mode(ColorBlock.black, BlendMode.srcIn),
      );
    }

    if (url?.endsWith('.avif') == true) {
      return CachedNetworkAvifImage(
        url != null && url!.isNotEmpty ? url! : Assets.noImageUrl,
        width: width,
        height: height,
        fit: fit,
