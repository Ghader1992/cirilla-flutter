...
import 'reset_password_store.dart';
...
abstract class AuthStoreBase with Store {
  ...
  late CouponReferralStore couponReferralStore;
  late UploadAvatarStore uploadAvatarStore;
  ...
  Future<void> loginSuccess(Map<String, dynamic> data) async {
    try {
      ...
      await updateTokenToDatabase(_requestHelper, token, topics: user?.roles, userId: user?.id, email: user?.userEmail);
    } catch (e) {
  ...
  AuthStoreBase(this._persistHelper, this._requestHelper) {
    ...
    couponReferralStore = CouponReferralStore(_requestHelper);
    uploadAvatarStore = UploadAvatarStore(_requestHelper, this as AuthStore);
    _init();
  }
