...
class Routes {
  ...
  static routes(SettingStore store) => <String, WidgetBuilder>{
        ...
        ForgotScreen.routeName: (context) => getGeneralConfig(store, ['forgotPasswordMethod'], 'link') == 'link'
            ? const ForgotScreen()
            : const ResetScreen(),
        ResetPasswordFormScreen.routeName: (context) => const ResetPasswordFormScreen(),
        ...
        CheckoutOnePage.routeName: (context) => const CheckoutOnePage(),
        ...
        BPMessageListScreen.routeName: (context) => BPMessageListScreen(store: store),
        BPForumListScreen.routeName: (context) => BPForumListScreen(store: store),
        BBPForumListScreen.routeName: (context) => BBPForumListScreen(store: store),
        BPActivityCreateScreen.routeName: (context) => const BPActivityCreateScreen(),
        BMMessageListScreen.routeName: (context) => BMMessageListScreen(store: store),
        PointSaleScreen.routeName: (context) => PointSaleScreen(store: store),
        CouponReferralScreen.routeName: (context) => const CouponReferralScreen(),
        PointAndRewardPage.routeName: (context) => const PointAndRewardPage(),
      };
  ...
  static onGenerateRoute(RouteSettings settings, SettingStore store) {
    ...
    // Ex: https://dev.rnlab.io/?ref=Q2634I9
    if (RegExp(r'\?ref=([_\-A-Za-z\d]+)$').hasMatch(uri.toString())) {
      String? refKey = RegExp(r'([_\-A-Za-z\d]+)$').stringMatch(uri.toString());
      return _getPageRoute(
    ...
    // Handling open tab
    // Ex: https://dev.rnlab.io/opentab?key=screens_cart
    if (RegExp(r'opentab\?key=([_\-A-Za-z\d]+)$').hasMatch(uri.toString())) {
      String? screen = RegExp(r'([_\-A-Za-z\d]+)$').stringMatch(uri.toString());
      store.setTab(screen);
    }
    ...
    switch (name) {
      ...
      case BPMemberListScreen.routeName:
        return _getPageRoute(screen: BPMemberListScreen(store: store, args: args), routeName: name);
      case BPMemberFriendScreen.routeName:
        return _getPageRoute(screen: BPMemberFriendScreen(store: store, args: args), routeName: name);
      case BPGroupListScreen.routeName:
        return _getPageRoute(screen: BPGroupListScreen(store: store, args: args), routeName: name);
      case BPActivityListScreen.routeName:
        return _getPageRoute(screen: BPActivityListScreen(store: store, args: args), routeName: name);
      case BPMemberDetailScreen.routeName:
        return _getPageRoute(screen: BPMemberDetailScreen(store: store, args: args), routeName: name);
      case BPChatMessageScreen.routeName:
        return _getPageRoute(screen: BPChatMessageScreen(store: store, args: args), routeName: name);
      case BPGroupDetailScreen.routeName:
        return _getPageRoute(screen: BPGroupDetailScreen(store: store, args: args), routeName: name);
      case BBPForumDetailScreen.routeName:
        return _getPageRoute(screen: BBPForumDetailScreen(store: store, args: args), routeName: name);
      case BBPTopicDetailScreen.routeName:
        return _getPageRoute(screen: BBPTopicDetailScreen(store: store, args: args), routeName: name);
      default:
        return null;
    }
  }
